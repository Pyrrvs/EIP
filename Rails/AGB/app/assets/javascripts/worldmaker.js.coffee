...
app.controller 'AppController', ['$scope', '$location', '$http', ($scope, $location, $http) ->
	app.scope = $scope
	app.scaling = 30
	$scope.view_mode = 'camera'
	$scope.game_state = 'stop'

	$http.get("#{window.location.pathname}/world.json").success (data) ->
		log "world", data
		$scope.world = data
	...
	$scope.save_world = ->
		...
		$http.put("#{window.location.pathname}/world", world: world).success (data) ->
